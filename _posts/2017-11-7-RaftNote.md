...
| 领导者完全原则  | 如果一个日志条目在某个任期被提交了，那么这个条目将出现在所有更高任期号的领导人日志。5.4 节 |
| 状态机安全原则  | 如果一个服务器状态机执行了给定日志索引位置的条目，那么其他服务器不会在相同索引位置上执行不同的日志条目。5.4.3 |
...
领导者决定何时将日志条目在状态机上执行是安全的。这样条目被称为已提交的（commited）。Raft 保证已提交的日志条目是持久化的，且最终被所有有效的状态机执行。一旦领导者创建的日志条目被复制到多数服务上，日志条目会被提交（如 Figure 6 中的条目7）。领导者日志中之前的日志条目也都是提交的，包括之前领导者创建的。领导者追踪记录他知道最大的已提交的索引，并且把这个索引包含在将来的 AppendEntries RPCs（包括心跳）中，以便于其它服务器知道领导者提交的位置。一旦跟随者知道一条日志条目已被提交，就会在本地状态机上执行条目（按照日志顺序）。
...
反证法，证明领导者完全原则。我们假设领导者完全原则不存在的，然后推论出矛盾。假设任期 T 的领导者(leaderT) 提交了一个日志条目在它的任期内，但是这条日志条目没有被存储在一些将来任期的领导者额日志中。设大于 T 的最小任期 U 的领导者 U (leaderU) 没有这条日志条目。

![Figure 9](../images/Raft-Leader-Log-Complete.png)

> Figure 9： 如果 S1 (任期 T 的领导者) 提交了一条新日志在它的任期内， 然后 S5 在之后的任期 U 内被选举为领导者，然后至会有一个机器，如 S3，即拥有来自 S1 的日志，也给 S5 投票了。

1. 在 领导者 U 选举的时候一定没有那条被提交的日志条目（领导者从不会删除或覆盖任何条目）
2. 领导者 T 复制了条目在多数的集群服务器上，并且领导者 U 收到集群中多数的投票。因此至少有一个服务器（选民) 同时接受了领导者 T 的日志条目和投票给领导者 U，如 Figure 9。这个选民是产生矛盾的关键点。
3. 这个选民必选先接受领导者 T 的提交的日志条目，在投票给领导者 U 之前。否则，它将拒绝领导者 T 的 AppendEntries 请求（当前任期可能比 T 大）。
4. 选民在给领导者 U 投票时依然包含这条日志，因为任何中间的领导者都包含该日志条目（根据上面假设 U 是最小的不包含这条日志的任期），领导者从不会删除条目，并且跟随者只能和领导者冲突时才会删除条目。
5. 选民投票给领导者 U，那么领导者 U的日志必须和选民一样新。这就导致了两者矛盾之一。
6. 首先，如果选民和领导者 U 的最后一条日志的任期号相同，那么领导者 U 的日志至少和选民一样长，所以领导者 U 的日志一定包含全部选民的日志条目。这是矛盾的，因为在假设中选民和领导者 U 包含的已提交日志是不同的。
7. 那么，如果领导者 U 的最后一条日志的任期号大于选民的，那么，也就大于 T了（因为选民的最后一条日志的任期号至少和 T 一样大）。根据假设，创建了领导者 U 最后一条日志条目的之前领导者一定包含了那条被提交的日志。所以，根据日志匹配原则，领导者 U 一定包含那条被提交的日志。这里产生矛盾。
8. 这里完成了矛盾。因此，所有比 T 大的领导者一定包含了所有来自 T 的已提交的日志。
9. 日志匹配原则，保证了未来的领导者也同时会包含被间接提交的条目，如 Figure 8的 e 中的索引2。

通过 领导者完全原则， 可以证明 Figure-3 中的状态机安全原则。在一个服务器应用一条日志条目到它自己的状态机中时，它的日志必须和领导者的日志在该条目和之前的条目上相同，并且被提交。考虑在任何一个服务器执行一个指定索引位置的日志的最小任期；领导者完全原则保证更高任期号的领导者会存储相同的日志条目，所以之后的任期里执行某个索引的日志条目也会是相同的值。因此，状态机安全原则成立。

最后， Raft 算法需要服务器按照日志中的索引顺序执行日志条目，和状态机安全原则结合看，这就意味着所有的服务器会执行相同的日志序列集到自己的状态机中，并且是按照相同的顺序的。

### 5.5 Follower and candidate crashes

如果跟随者和候选者崩溃，后续的 RequestVote 和 AppendEntries RPCs 将失败。Raft 处理这些错误通过简单的一直重试处理。同时 Raft RPCs 是幂等的，所有重复的请求是没有影响的。

### 5.6 Timing and availability

Raft 的要求之一就是安全性不能依赖时间：整个系统不能因为某些事件运行的比预期的快一点或慢一点就产生了错误的结果。但是，可用性（系统可以即时响应客户端）不可避免的要依赖时间。

领导者选举是 Raft 中对时间要求最关键的部分。 下列公式要求，用于保证 Raft 可以选举出并维持一个稳定的领导者： ```广播时间（broadcastTime）<< 选举超时时间（electionTimeOut) << 平均故障间隔时间（MTBF)```

- 广播时间：一个服务器并行地发送 RPCs 给集群中的其他服务器并响应的平均时间。
- 选举超时时间：5.2 节中的说明的选举超时时间。
- 平均故障间隔时间：一台服务器，两次故障之间的平均时间

广播时间必须比选举超时时间小一个数量级，这样领导者才能够稳定地发送心跳来阻止跟随者进入选举状态。通过随机化选举超时时间的方式，这个不等式也使得选票瓜分的情况变的不可能。选举超时时间应该比平均故障时间小上几个数量级，这样这个系统才能稳定运行。当领导者崩溃时，整个系统不可用的时间相当于选举超时时间内。

广播时间和平均故障间隔时间有系统决定，但是选举超时时间有我们选择。Raft 的RPCs 需要接受方将信息持久化的保持到稳定存储中去，所以广播时间大约是 0.5ms - 20ms，取决于存储技术。因此选举超时时间可能需要在 10ms - 500ms 之间。多数服务器的 MTBF 在几个月或更长，很好满足。
...
3. 一旦$C_{old, new}$被提交，$C_{old}$ 和 $C_{new}$ 都无法做出决定没有彼此的同意，而且根据领导者完全原则可以确保只有有 $C_{old, new}$ 日志可以被选举为领导者。这时，领导者创建$C_{new}$ 日志条目并复制给集群就是安全的了。另外，每个服务器在收到新的配置的时候就会立即生效。当新配置在$C_{new}$ 的规则下被提交，旧的配置就变的无关紧要了，同时不使用新的配置的服务器就可以被关闭了。如 Figure-11，任何时刻$C_{old}$ 和 $C_{new}$ 不可以单独做出决定。这样保证了安全性。

重新配置提出的3个问题:

1. 新加入的服务可能没有任何日志条目，这种状态下加入集群，可能导致浪费时间追赶，而无法提交新的日志。为了避免这种可用性的间隔时间，Raft 添加一种额外的阶段在配置变更前，新的服务器以非投票成员的方式加入(领导者复制日志，当不被考虑到大多数中)。一旦新的服务器赶上集群的其他成员，上面描述的方式进行重新配置。



### 7 Log compaction

Raft 的日志在正常操作下持续增加，随着客户端的请求，实际当中不可能无边界的增加。随着日志的增加，将占用更多的空间，和或更多时间重演。如果没有机制去废弃积累在日志中无用的信息，将导致可用性问题。

快照是一种简单的压缩方式。在快照中，整个系统的状态都被写入到快照中，存储到持久化的存储中，然后废弃某个时刻之前的日志
